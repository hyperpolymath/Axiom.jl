...
## TASK 7: Fix SMT Extension Dead Code
...
cd /var/mnt/eclipse/repos/Axiom.jl
julia --project=. -e '
# Just verify no syntax errors and module loads
using Axiom
println("SMT extension compiles without error")
# If SMTLib.jl is available, test deeper:
# using SMTLib
# result = Axiom.prove(:(x > 0 && x < 10 => x >= 0))
'
```

---

## TASK 8: Fix Zig Backend — Either Implement or Remove
...
**Option A: Implement** (if Zig backend adds value alongside Rust backend)
1. Build the Zig library in `zig/` with `zig build`
2. Implement actual `ccall` bindings in `zig_ffi.jl` for matmul, activations, conv2d
3. Set `ZIG_LIB` to the correct built library path

**Option B (RECOMMENDED): Remove**
1. Delete `src/backends/zig_ffi.jl`
2. Remove Zig backend references from `src/Axiom.jl`
3. Keep the `zig/` directory if it has useful standalone code, but don't pretend it's integrated
4. Update any docs that claim Zig backend support
...
julia --project=. -e '
using Axiom
@assert !isdefined(Axiom, :zig_matmul) "Zig FFI not cleanly removed"
println("ZIG BACKEND CLEANLY REMOVED")
'
```
