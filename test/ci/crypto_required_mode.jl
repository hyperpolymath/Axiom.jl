...
@testset "Crypto strict mode compile gate" begin
    model = Sequential(
        Dense(6, 4, relu),
        Dense(4, 3),
        Softmax(),
    ...
    with_env(Dict(
        ...
        @test occursin("AXIOM_CRYPTO_REQUIRED", sprint(showerror, err))
    end
end

@testset "Crypto strict mode runtime with built-in production kernels" begin
    ...
    with_env(Dict(
        ...
        dense_compiled = compile(dense_model, backend = CryptoBackend(0), verify = false, optimize = :none)
        ...
        conv_compiled = compile(conv_model, backend = CryptoBackend(0), verify = false, optimize = :none)
        ...
        norm_compiled = compile(norm_model, backend = CryptoBackend(0), verify = false, optimize = :none)
        ...
        avgpool_compiled = compile(avgpool_model, backend = CryptoBackend(0), verify = false, optimize = :none)
        ...
        for hook in (
            ...
            @test crypto["hook_overrides"][hook] == true
        end

        diag = coprocessor_runtime_diagnostics()["backends"]["crypto"]
