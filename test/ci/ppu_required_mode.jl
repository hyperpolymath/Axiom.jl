...
@testset "PPU strict mode runtime hook gate" begin
    model = Sequential(
        Dense(6, 4, relu),
        Dense(4, 3),
        Softmax(),
    ...
    with_env(Dict(
        ...
        @test occursin("AXIOM_PPU_REQUIRED", msg)
        @test occursin("strict mode enabled", msg)
    end
end

# Install minimal PPU hook overrides to demonstrate strict-mode success path.
@eval begin
...
@testset "PPU strict mode with hook overrides" begin
    model = Sequential(
        Dense(6, 4, relu),
        Dense(4, 3),
        Softmax(),
    ...
    with_env(Dict(
        ...
        @test ppu["hook_overrides"]["backend_coprocessor_matmul"] == true
        @test ppu["hook_overrides"]["backend_coprocessor_relu"] == true
        @test ppu["hook_overrides"]["backend_coprocessor_softmax"] == true
    end
end
