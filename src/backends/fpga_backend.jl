...
# Axiom.jl FPGA Coprocessor Backend
#
# Built-in production kernels for FPGABackend. These methods are intentionally
...
function _fpga_as_strided_matrix(A::AbstractMatrix{T}) where {T<:AbstractFloat}
    A isa StridedMatrix{T} ? A : Matrix{T}(A)
end

function _fpga_backend_matmul(
    A::AbstractMatrix{T},
    B::AbstractMatrix{T},
) where {T<:AbstractFloat}
    size(A, 2) == size(B, 1) || throw(DimensionMismatch("matmul size mismatch: $(size(A)) x $(size(B))"))
    A_mat = _fpga_as_strided_matrix(A)
    B_mat = _fpga_as_strided_matrix(B)
...
function _fpga_softmax_dim2(x::AbstractMatrix{T}) where {T<:AbstractFloat}
    rows, cols = size(x)
    y = similar(x)
    cols == 0 && return y
...
function _fpga_softmax_dim1(x::AbstractMatrix{T}) where {T<:AbstractFloat}
    rows, cols = size(x)
    y = similar(x)
    rows == 0 && return y
...
function _fpga_backend_softmax(
    x::AbstractArray{T},
    dim::Int,
) where {T<:AbstractFloat}
    if x isa AbstractMatrix{T}
        dim == 2 && return _fpga_softmax_dim2(x)
        dim == 1 && return _fpga_softmax_dim1(x)
    end
