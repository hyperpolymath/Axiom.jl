...
- [ ] Non-GPU accelerators (TPU/NPU/DSP/FPGA) backend strategy (in progress: targets, detection, compiled dispatch, fallback/strategy CI, capability/evidence reporting, runtime self-healing diagnostics, and resilience CI/evidence shipped via `coprocessor_capability_report`, `coprocessor_runtime_diagnostics`, `scripts/coprocessor-evidence.jl`, `test/ci/coprocessor_resilience.jl`, and `scripts/coprocessor-resilience-evidence.jl`; production kernels remain).
...
Readiness verification (2026-02-17): `scripts/readiness-check.sh` => Passed: 17, Failed: 0, Skipped: 0.
