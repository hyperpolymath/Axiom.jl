...
- [ ] Non-GPU accelerators (TPU/NPU/PPU/MATH/CRYPTO/FPGA/DSP) backend strategy (in progress: targets, detection, compiled dispatch, fallback/strategy CI, capability/evidence reporting, runtime self-healing diagnostics, resilience CI/evidence, strict-mode gating shipped for TPU/NPU/PPU/MATH/CRYPTO/FPGA/DSP, and Math/PPU/Crypto/FPGA built-in production kernels/evidence shipped via `src/backends/math_backend.jl`, `src/backends/ppu_backend.jl`, `src/backends/crypto_backend.jl`, `src/backends/fpga_backend.jl`, `test/ci/math_required_mode.jl`, `test/ci/ppu_required_mode.jl`, `test/ci/crypto_required_mode.jl`, `test/ci/fpga_required_mode.jl`, `scripts/math-strict-evidence.jl`, `scripts/ppu-strict-evidence.jl`, `scripts/crypto-strict-evidence.jl`, `scripts/fpga-strict-evidence.jl`, `scripts/math-production-evidence.jl`, `scripts/ppu-production-evidence.jl`, `scripts/crypto-production-evidence.jl`, and `scripts/fpga-production-evidence.jl`; production kernels remain for TPU/NPU/DSP).
- [ ] Accelerator rollout sequence (2026-02-18): Math + PPU + Crypto + FPGA production-ready baselines shipped; VPU + QPU basics next; remaining accelerator production hardening targeted for v2.
...
Readiness verification (2026-02-18): `scripts/readiness-check.sh` => Passed: 41, Failed: 0, Skipped: 0.
