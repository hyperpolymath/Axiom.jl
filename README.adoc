...
=== Model Interoperability (Roadmap)

[source,julia]
----
# Build a model and verify it in Axiom directly
model = Sequential(Dense(784, 128, relu), Dense(128, 10), Softmax())
input = Tensor(randn(Float32, 16, 784))
output = model(input)
result = verify(model, properties=[ValidProbabilities(), FiniteOutput()], data=[(input, nothing)])
----

PyTorch/ONNX import paths are tracked as future work and are not yet a stable public API.
