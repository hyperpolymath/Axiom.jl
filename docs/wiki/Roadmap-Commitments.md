...
| TPU/NPU/PPU/MATH/CRYPTO/FPGA/DSP backends | In progress (targets + strategy CI + capability/evidence reporting + runtime self-healing diagnostics/resilience evidence + TPU/NPU/PPU/MATH/CRYPTO/FPGA/DSP strict-mode gates shipped; Math/PPU/Crypto/FPGA production kernels/evidence shipped) | Stage 5+ | At least one production-grade non-GPU accelerator backend with strict-mode fallback controls, CI coverage, resilience diagnostics, and benchmark evidence |
| Accelerator rollout sequence | In progress (Math + PPU + Crypto + FPGA production-ready baselines shipped; VPU/QPU basics queued; remaining production hardening in v2) | Stage 5+ | Keep roadmap/docs/CI aligned with the sequencing policy and close each stage with evidence artifacts |
...
Latest readiness verification (2026-02-18): `scripts/readiness-check.sh` => Passed: 41, Failed: 0, Skipped: 0.
