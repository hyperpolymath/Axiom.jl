...
| TPU/NPU/PPU/MATH/CRYPTO/FPGA/DSP backends | In progress (targets + strategy CI + capability/evidence reporting + runtime self-healing diagnostics/resilience evidence + TPU/NPU/PPU/MATH/CRYPTO/DSP strict-mode gates shipped) | Stage 5+ | At least one production-grade non-GPU accelerator backend with strict-mode fallback controls, CI coverage, resilience diagnostics, and benchmark evidence |
