...
def call(sym, x):
    ...
    fn(x.ctypes.data_as(f32p), y.ctypes.data_as(f32p), x.size)
    return y
...
rt = np.tanh(xs.astype(np.float64)).astype(np.float32)
rg = gelu_ref(xs)
...
xt = xs.copy()
fn = lib.axiom_tanh_inplace
fn.restype=None
fn.argtypes=[f32p, ctypes.c_size_t]
fn(xt.ctypes.data_as(f32p), xt.size)
...
for sigma in (1, 3, 5, 8):
    ...
    g = call('axiom_gelu', pre)
    print(f"gelu on N(0,{sigma}²) x1e6: NaN count = {np.isnan(g).sum()}, max|x|={np.abs(pre).max():.1f}")
