...
function strict_runtime_probe()
    ...
    with_env(Dict(
        ...
        Dict(
            ...
            "kernel_hooks_loaded" => math["kernel_hooks_loaded"],
...
function main()
    ...
    (strict_runtime["required"] == true) || push!(failures, "Math strict runtime probe did not report required=true")
    (strict_runtime["kernel_hooks_loaded"] == true) || push!(failures, "Math strict runtime probe did not report full hook coverage")
    for key in ("dense", "conv", "norm", "avgpool")
        probe = strict_runtime[key]
        (probe["compiled_wrapper"] == true) || push!(failures, "Math strict runtime probe for $key did not compile to coprocessor wrapper")
        (probe["parity_ok"] == true) || push!(failures, "Math strict runtime probe parity check failed for $key")
        (probe["finite_ok"] == true) || push!(failures, "Math strict runtime finite check failed for $key")
        (probe["probability_ok"] == true) || push!(failures, "Math strict runtime probability check failed for $key")
    end
    ...
    for hook in (
        ...
        Bool(hooks[hook]) || push!(failures, "Math hook override not detected for $hook")
    end

    diagnostics = strict_runtime["runtime_diagnostics"]
    (diagnostics["compile_fallbacks"] == 0) || push!(failures, "Math compile fallback counter should remain 0 in strict runtime probe")
    (diagnostics["runtime_errors"] == 0) || push!(failures, "Math runtime error counter should remain 0 in strict runtime probe")
    (diagnostics["runtime_fallbacks"] == 0) || push!(failures, "Math runtime fallback counter should remain 0 in strict runtime probe")
    (diagnostics["recoveries"] == 0) || push!(failures, "Math recovery counter should remain 0 in strict runtime probe")

    payload = Dict(
        "format" => "axiom-math-strict-evidence.v2",
