...
function missing_hook_probe(model, x)
    with_env(Dict(
        ...
        compiled = compile(model, backend = PPUBackend(0), verify = false, optimize = :none)
...
function install_ppu_demo_hooks!()
    @eval begin
...
function main()
    ...
    (missing_hook["compiled_wrapper"] == true) || push!(failures, "PPU strict missing-hook probe did not compile to coprocessor wrapper")
    (missing_hook["raised_error"] == true) || push!(failures, "PPU strict missing-hook probe did not raise an error")
    if missing_hook["error_message"] !== nothing
        occursin("strict mode enabled", missing_hook["error_message"]) ||
            push!(failures, "PPU strict missing-hook probe error message missing strict-mode wording")
    end

    install_ppu_demo_hooks!()
    strict_ok = strict_success_probe(model, x, cpu)

    (strict_ok["compiled_wrapper"] == true) || push!(failures, "PPU strict success probe did not compile to coprocessor wrapper")
    (strict_ok["required"] == true) || push!(failures, "PPU strict success probe did not report required=true")
    (strict_ok["parity_ok"] == true) || push!(failures, "PPU strict success probe parity check failed")
    (strict_ok["finite_ok"] == true) || push!(failures, "PPU strict success probe finite check failed")
    (strict_ok["probability_ok"] == true) || push!(failures, "PPU strict success probe probability check failed")

    hooks = strict_ok["hook_overrides"]
    Bool(hooks["backend_coprocessor_matmul"]) || push!(failures, "PPU hook override not detected for matmul")
    Bool(hooks["backend_coprocessor_relu"]) || push!(failures, "PPU hook override not detected for relu")
    Bool(hooks["backend_coprocessor_softmax"]) || push!(failures, "PPU hook override not detected for softmax")

    payload = Dict(
        "format" => "axiom-ppu-strict-evidence.v1",
