...
function load_baseline(path::String)
    ...
    try
        ...
        @warn "Failed to parse math baseline file; ignoring baseline checks" path = path exception = (err, catch_backtrace())
        Dict{String, Any}()
    end
end

function baseline_p50_ms(baseline::Dict{String, Any})
    haskey(baseline, "math_p50_ms") || return nothing
    value = tryparse(Float64, string(baseline["math_p50_ms"]))
    value === nothing ? nothing : value
end

function main()
    required = parse_bool_env("AXIOM_MATH_REQUIRED", true)
    enforce_regression = parse_bool_env("AXIOM_MATH_BASELINE_ENFORCE", false)
    warmup = parse_int_env("AXIOM_MATH_PERF_WARMUP", 3)
    iterations = parse_int_env("AXIOM_MATH_PERF_ITERATIONS", 20)
    max_ratio = parse_float_env("AXIOM_MATH_MAX_REGRESSION_RATIO", 1.20)
    baseline_path = get(ENV, "AXIOM_MATH_BASELINE_PATH", joinpath(pwd(), "benchmark", "math_performance_baseline.json"))
    out_path = get(ENV, "AXIOM_MATH_PRODUCTION_EVIDENCE_PATH", joinpath(pwd(), "build", "math_production_evidence.json"))
    ...
    result = with_env(Dict(
        ...
        "AXIOM_MATH_REQUIRED" => required ? "1" : "0",
    )) do
        ...
        diagnostics = coprocessor_runtime_diagnostics()["backends"]["math"]
        ...
        Dict(
            ...
            "hook_coverage_ok" => Bool(math["kernel_hooks_loaded"]) && all(values(math["hook_overrides"])),
            "required" => Bool(math["required"]),
    ...
    if baseline_p50 === nothing
        ...
        if enforce_regression && regressed
            push!(failures, "Math regression ratio $(round(ratio; digits = 3)) exceeds $(round(max_ratio; digits = 3))")
        end
    end

    result["compiled_wrapper"] || push!(failures, "Math backend did not produce a coprocessor compiled wrapper")
    result["parity_ok"] || push!(failures, "Math backend parity mismatch against CPU reference")
    result["finite_ok"] || push!(failures, "Math backend produced non-finite outputs")
    result["probability_ok"] || push!(failures, "Math backend output probabilities are invalid")
    result["hook_coverage_ok"] || push!(failures, "Math backend hook coverage is incomplete")

    diagnostics = result["runtime_diagnostics"]
    Int(diagnostics["compile_fallbacks"]) == 0 || push!(failures, "Math compile fallback counter is non-zero")
    Int(diagnostics["runtime_errors"]) == 0 || push!(failures, "Math runtime error counter is non-zero")
    Int(diagnostics["runtime_fallbacks"]) == 0 || push!(failures, "Math runtime fallback counter is non-zero")

    payload = Dict(
        "format" => "axiom-math-production-evidence.v1",
        ...
        "math_capabilities" => coprocessor_capability_report()["backends"]["MATH"],
        "math_backend_result" => result,
    ...
    println("math production evidence written: $out_path")
    if !isempty(failures)
        println("math production evidence checks failed:")
