...
function load_baseline(path::String)
    ...
    try
        ...
        @warn "Failed to parse PPU baseline file; ignoring baseline checks" path = path exception = (err, catch_backtrace())
        Dict{String, Any}()
    end
end

function baseline_p50_ms(baseline::Dict{String, Any})
    haskey(baseline, "ppu_p50_ms") || return nothing
    value = tryparse(Float64, string(baseline["ppu_p50_ms"]))
    value === nothing ? nothing : value
end

function main()
    required = parse_bool_env("AXIOM_PPU_REQUIRED", true)
    enforce_regression = parse_bool_env("AXIOM_PPU_BASELINE_ENFORCE", false)
    warmup = parse_int_env("AXIOM_PPU_PERF_WARMUP", 3)
    iterations = parse_int_env("AXIOM_PPU_PERF_ITERATIONS", 20)
    max_ratio = parse_float_env("AXIOM_PPU_MAX_REGRESSION_RATIO", 1.20)
    baseline_path = get(ENV, "AXIOM_PPU_BASELINE_PATH", joinpath(pwd(), "benchmark", "ppu_performance_baseline.json"))
    out_path = get(ENV, "AXIOM_PPU_PRODUCTION_EVIDENCE_PATH", joinpath(pwd(), "build", "ppu_production_evidence.json"))
    ...
    result = with_env(Dict(
        ...
        "AXIOM_PPU_REQUIRED" => required ? "1" : "0",
    )) do
        ...
        Dict(
            ...
            "hook_coverage_ok" => Bool(ppu["kernel_hooks_loaded"]) && all(values(ppu["hook_overrides"])),
            "required" => Bool(ppu["required"]),
    ...
    if baseline_p50 === nothing
        ...
        if enforce_regression && regressed
            push!(failures, "PPU regression ratio $(round(ratio; digits = 3)) exceeds $(round(max_ratio; digits = 3))")
        end
    end

    result["compiled_wrapper"] || push!(failures, "PPU backend did not produce a coprocessor compiled wrapper")
    result["parity_ok"] || push!(failures, "PPU backend parity mismatch against CPU reference")
    result["finite_ok"] || push!(failures, "PPU backend produced non-finite outputs")
    result["probability_ok"] || push!(failures, "PPU backend output probabilities are invalid")
    result["hook_coverage_ok"] || push!(failures, "PPU backend hook coverage is incomplete")

    diagnostics = result["runtime_diagnostics"]
    Int(diagnostics["compile_fallbacks"]) == 0 || push!(failures, "PPU compile fallback counter is non-zero")
    Int(diagnostics["runtime_errors"]) == 0 || push!(failures, "PPU runtime error counter is non-zero")
    Int(diagnostics["runtime_fallbacks"]) == 0 || push!(failures, "PPU runtime fallback counter is non-zero")

    payload = Dict(
        "format" => "axiom-ppu-production-evidence.v1",
        ...
        "ppu_capabilities" => coprocessor_capability_report()["backends"]["PPU"],
        "ppu_backend_result" => result,
    ...
    println("ppu production evidence written: $out_path")
    if !isempty(failures)
        println("ppu production evidence checks failed:")
