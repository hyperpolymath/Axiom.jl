...
function load_baseline(path::String)
    ...
    try
        ...
        @warn "Failed to parse Crypto baseline file; ignoring baseline checks" path = path exception = (err, catch_backtrace())
        Dict{String, Any}()
    end
end

function baseline_p50_ms(baseline::Dict{String, Any})
    haskey(baseline, "crypto_p50_ms") || return nothing
    value = tryparse(Float64, string(baseline["crypto_p50_ms"]))
    value === nothing ? nothing : value
end

function main()
    required = parse_bool_env("AXIOM_CRYPTO_REQUIRED", true)
    enforce_regression = parse_bool_env("AXIOM_CRYPTO_BASELINE_ENFORCE", false)
    warmup = parse_int_env("AXIOM_CRYPTO_PERF_WARMUP", 3)
    iterations = parse_int_env("AXIOM_CRYPTO_PERF_ITERATIONS", 20)
    max_ratio = parse_float_env("AXIOM_CRYPTO_MAX_REGRESSION_RATIO", 1.20)
    baseline_path = get(ENV, "AXIOM_CRYPTO_BASELINE_PATH", joinpath(pwd(), "benchmark", "crypto_performance_baseline.json"))
    out_path = get(ENV, "AXIOM_CRYPTO_PRODUCTION_EVIDENCE_PATH", joinpath(pwd(), "build", "crypto_production_evidence.json"))
    ...
    result = with_env(Dict(
        ...
        "AXIOM_CRYPTO_REQUIRED" => required ? "1" : "0",
    )) do
        ...
        Dict(
            ...
            "hook_coverage_ok" => Bool(crypto["kernel_hooks_loaded"]) && all(values(crypto["hook_overrides"])),
            "required" => Bool(crypto["required"]),
    ...
    if baseline_p50 === nothing
        ...
        if enforce_regression && regressed
            push!(failures, "Crypto regression ratio $(round(ratio; digits = 3)) exceeds $(round(max_ratio; digits = 3))")
        end
    end

    result["compiled_wrapper"] || push!(failures, "Crypto backend did not produce a coprocessor compiled wrapper")
    result["parity_ok"] || push!(failures, "Crypto backend parity mismatch against CPU reference")
    result["finite_ok"] || push!(failures, "Crypto backend produced non-finite outputs")
    result["probability_ok"] || push!(failures, "Crypto backend output probabilities are invalid")
    result["hook_coverage_ok"] || push!(failures, "Crypto backend hook coverage is incomplete")

    diagnostics = result["runtime_diagnostics"]
    Int(diagnostics["compile_fallbacks"]) == 0 || push!(failures, "Crypto compile fallback counter is non-zero")
    Int(diagnostics["runtime_errors"]) == 0 || push!(failures, "Crypto runtime error counter is non-zero")
    Int(diagnostics["runtime_fallbacks"]) == 0 || push!(failures, "Crypto runtime fallback counter is non-zero")

    payload = Dict(
        "format" => "axiom-crypto-production-evidence.v1",
        ...
        "crypto_capabilities" => coprocessor_capability_report()["backends"]["CRYPTO"],
        "crypto_backend_result" => result,
    ...
    println("crypto production evidence written: $out_path")
    if !isempty(failures)
        println("crypto production evidence checks failed:")
